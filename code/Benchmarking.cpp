...
// This is a simple client executable used to benchmark performance of parallel MPC routines.
...
int main(int argc, char** argv) {
  if (argc < 4) {
    cout << "Usage: ParallelTesting party_id param_file num_elements [num_trials (optional)]" << endl;
    return 1;
  }
  ...
  // Use this variable to specify the different parallelism levels we'll use to measure runtime
  ...
  // This optional variable controls the number of trials to average runtime measurements across
  // The default value is 5 trials
  int n_trials = 5;
  if (argc == 5) {
    string n_tr(argv[4]);
    n_trials = stoi(n_tr);
  ...
  // flags to control which routines to benchmark and whether or not to print the actual output of the routines
  bool fpdiv = true;
  bool fpsqrt = true;
  bool ispos = true;
  bool orth = true;
